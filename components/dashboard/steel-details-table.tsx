...
type SteelFormValues = z.infer<typeof steelSchema>;
...
export function SteelDetailsTable() {
  ...
  const [activeRecord, setActiveRecord] = useState<SteelFormValues | null>(null);
  const [recordToDelete, setRecordToDelete] = useState<string | null>(null);

  const form = useForm<SteelFormValues>({
    resolver: zodResolver(steelSchema),
    defaultValues: {
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: SteelFormValues) => {
      const isNew = !data.id;
      const payload = {
        ...data,
        billetWeight: Number(data.billetWeight),
        billetQty: Number(data.billetQty),
        steelWeight: Number(data.steelWeight),
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['steel-details'] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/steel-details?id=${id}`, { method: 'DELETE' });
  ...
  const onSubmit = (data: SteelFormValues) => {
  ...
  const columns: ColumnDef<any>[] = [
    ...
    {
      accessorKey: "billetWeight",
      header: "Billet Wt (T)",
      cell: ({ row }) => <span className="text-foreground font-mono whitespace-nowrap">{row.original.billetWeight} T</span>,
    },
    {
      accessorKey: "steelSize",
      header: "Steel Size",
      cell: ({ row }) => <span className="font-semibold text-foreground whitespace-nowrap">{row.original.steelSize}</span>,
    },
    {
      accessorKey: "steelWeight",
      header: "Steel Wt (T)",
      cell: ({ row }) => <span className="font-semibold text-cyan-600 dark:text-cyan-600 dark:text-cyan-400 font-mono whitespace-nowrap">{row.original.steelWeight} T</span>,
    },
    {
      accessorKey: "dispatchDate",
      header: "Dispatch",
      cell: ({ row }) => <span className="text-xs text-muted-foreground whitespace-nowrap">{row.original.dispatchDate}</span>,
    },
    {
      accessorKey: "status",
      header: "Status",
      cell: ({ row }) => (
        <span className={cn(
          ...
          statusColors[row.original.status] || "text-slate-500 bg-slate-50 border-slate-200 dark:text-slate-400 dark:bg-slate-400/10 dark:border-slate-400/20"
        )}>
          {row.original.status}
        </span>
      ),
    ...
    <>
      ...
      >
        ...
        <DataTable 
          ...
          isLoading={isLoading} 
          onRefresh={refetch}
          exportFilename="Steel_Details_Export"
          searchPlaceholder="Search steel records..."
        />
      </motion.div>
            ...
            >
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="p-5 space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input {...form.register("customerName")} type="text" placeholder="e.g. Acme Corp" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground placeholder:text-gray-400" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input {...form.register("billetSize")} type="text" placeholder="e.g. 150x150" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground placeholder:text-gray-400" />
                    {form.formState.errors.billetSize && <span className="text-[10px] text-red-500">{form.formState.errors.billetSize.message}</span>}
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground">Steel Size</label>
                    <input {...form.register("steelSize")} type="text" placeholder="e.g. 240mm" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground placeholder:text-gray-400" />
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input {...form.register("billetWeight")} type="number" step="0.01" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                    {form.formState.errors.billetWeight && <span className="text-[10px] text-red-500">{form.formState.errors.billetWeight.message}</span>}
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground">Billet Qty</label>
                    <input {...form.register("billetQty")} type="number" step="1" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                    {form.formState.errors.billetQty && <span className="text-[10px] text-red-500">{form.formState.errors.billetQty.message}</span>}
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground">Steel Wt (T)</label>
                    <input {...form.register("steelWeight")} type="number" step="0.01" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                    {form.formState.errors.steelWeight && <span className="text-[10px] text-red-500">{form.formState.errors.steelWeight.message}</span>}
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground">Steel Qty</label>
                    <input {...form.register("steelQty")} type="number" step="1" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input {...form.register("rollingDate")} type="date" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                    {form.formState.errors.rollingDate && <span className="text-[10px] text-red-500">{form.formState.errors.rollingDate.message}</span>}
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground">Cutting Date</label>
                    <input {...form.register("cuttingDate")} type="date" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                    {form.formState.errors.cuttingDate && <span className="text-[10px] text-red-500">{form.formState.errors.cuttingDate.message}</span>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input {...form.register("dispatchDate")} type="date" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                    {form.formState.errors.dispatchDate && <span className="text-[10px] text-red-500">{form.formState.errors.dispatchDate.message}</span>}
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-muted-foreground">Actual Dispatch</label>
                    <input {...form.register("actualDispatchDate")} type="text" placeholder="YYYY-MM-DD or '—'" className="w-full px-3 py-2 text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/50 text-foreground" />
                  </div>
                  <div className="space-y-1.5">
