    ...
    >
      ...
      <div className="flex h-16 items-center px-4 border-b border-white/8 flex-shrink-0 relative z-10 gap-3">
        ...
        <div className="space-y-1.5">
          <Skeleton className="h-3.5 w-24 bg-gray-100 dark:bg-white/10" />
      ...
      <div className="flex-1 overflow-y-auto py-6 px-3 space-y-6 relative z-10">
        {Array.from({ length: 3 }).map((group, gIdx) => (
          <div key={gIdx} className="space-y-3">
            {/* Nav Group Label */}
            <Skeleton className="h-2.5 w-16 bg-gray-50 dark:bg-white/5 ml-3" />
            ...
            <div className="space-y-1.5">
              {Array.from({ length: gIdx === 0 ? 2 : gIdx === 1 ? 3 : 2 }).map((item, iIdx) => (
                ...
                >
                  {/* Icon circle */}
                  <Skeleton className="h-5 w-5 rounded bg-gray-100 dark:bg-white/10 flex-shrink-0" />
                  {/* Label text */}
                  <Skeleton className="h-3 w-28 bg-gray-50 dark:bg-white/5" />
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="border-t border-white/8 p-4 flex-shrink-0 relative z-10">
        <div className="flex items-center gap-3 rounded-xl p-1 bg-white/[0.01]">
          ...
          <div className="relative">
            <Skeleton className="h-8 w-8 rounded-full bg-white/10 border border-gray-200 dark:border-white/10" />
